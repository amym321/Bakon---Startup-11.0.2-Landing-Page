...
class StaticPassword {
  constructor(section) {
    ...
    this.$newsletterForm = jquery_default()('.newsletter-section', this.$el);
    this.$newsletterToggle = jquery_default()('[data-newsletter-toggle]', this.$el);
    this.$passwordForm = jquery_default()('[data-password-form]', this.$el);
    this.$passwordToggle = jquery_default()('[data-password-toggle]', this.$el);
    jquery_default()('[data-password-trigger]', this.$el).on('click', event => this.togglePasswordForm(event));
    this.hasErrors = this.$passwordForm.find('.has-errors').length;
    dist_index_es.init();

    if (this.hasErrors) {
      this.togglePasswordForm(false, true);
    }
  }

  togglePasswordForm(event, hideNewsletter) {
    if (event == null) {
      event = false;
    }
    ...
    if (this.$passwordForm.hasClass('visible' || 0)) {
      ...
      this.$passwordToggle.addClass('hidden').removeClass('visible');
    } else {
      ...
      this.$newsletterToggle.addClass('hidden').removeClass('visible');
    }
  }

}
jquery_default()(() => window.theme = new StaticPassword({
  el: document.body
}));
})();
...
/******/ })()
;
//# sourceMappingURL=startup-password.js.map?1658331185241